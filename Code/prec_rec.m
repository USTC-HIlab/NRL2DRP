function [prec, tpr, fpr, thresh,aupr] = prec_rec(score, target, name, varargin)
% score = score_predict_three;target = T_true;varargin = color0;
...
if (plot_pr || plot_roc)
    ...
    if (~hold_fig)
        ...
        if (plot_pr)
            ...
            plot([0; tpr], [1 ; prec], 'Color',style,'LineWidth',2); % add pseudo point to complete curve
        ...
        if (plot_roc)
            ...
            plot([0; fpr], [0; tpr],'Color', style,'LineWidth',2); % add pseudo point to complete curve
        ...
        if (plot_roc)
            ...
            plot([0; fpr], [0; tpr],'Color', style,'LineWidth',2);
...
% backcolor=[233/256 233/256 233/256];
% set(gca, 'color', backcolor);
set(gcf,'color','white');

%补充计算的AUPR值，其实就是AP(Approximately Average precision值)
num = size(prec,1);
aupr = 0;recall = [0; tpr];
for j = 1:num
    aupr = aupr + prec(j)*(recall(j+1)-recall(j));
end
